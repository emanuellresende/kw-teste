...
export default function HomeScreen() {
  ...
  async function onHandleSubmit() {
    ...
    const data = await axios.post('http://localhost:3000/api/login', { accessToken: token, email });
    if (data.data.status === 400) {
      ...
      setAccept(1);
      alert("Seu Token de acesso unico é: http://localhost:3001/users/" + email + '/' + data.data.data.newAccessToken);
    }
