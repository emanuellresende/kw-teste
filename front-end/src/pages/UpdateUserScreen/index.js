...
export default function UpdateUserScreen() {
  ...
  async function loading() {
    ...
    const data = await axios.post('http://localhost:3000/api/login', { accessToken: location.state.accessToken, email: location.state.email });
  ...
  async function onHandleSubmit(e) {
    ...
    if (validateName() === false) {
      ...
      const data = await axios.patch('http://localhost:3000/api/update', { name: username, email: location.state.email });
      if (data.data.status === 400) {
